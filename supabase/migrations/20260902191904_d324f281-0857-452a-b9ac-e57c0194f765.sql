...
DECLARE
  v_a uuid; v_b uuid;
  v_ta uuid := '901fa791-5322-46dc-95fd-3bb50a37e08d';
  v_tb uuid := '3588643d-a2a8-4093-89b8-ed542eaa5c25';
  v_dir uuid := '30b6499b-11eb-40d0-b43c-ed0c191ea52d';
...
BEGIN
  ...
  INSERT INTO ab_results VALUES
    ('professor A le a propria escola', n1 > 0, 'alunos A visiveis: '||n1),
    ('professor A nao le alunos de B', n2 = 0, 'alunos B visiveis: '||n2),
    ('professor A nao le freq/ocor/eventos/settings/IRA de B',
      n3+n4+n5+n6+n7 = 0, format('freq %s, ocor %s, ev %s, set %s, ira %s', n3,n4,n5,n6,n7)),
    ('professor A nao grava em B', b1, 'insert cross-school bloqueado'),
    ('professor A nao acessa Storage de B', b2 = false, 'storage_school_allowed(B)='||b2);
  ...
  END;
  b2 := public.storage_school_allowed('legacy-file.png');
  RESET ROLE;
  INSERT INTO ab_results VALUES
    ('professor B le a propria escola', n1 > 0, 'alunos B visiveis: '||n1),
    ('professor B nao le dados de A', n2+n3 = 0, format('alunos %s, freq %s', n2, n3)),
    ('professor B nao le atestados/PEI/notas/IRA de A',
      n4+n5+n6+n7 = 0, format('atest %s, pei %s, notas %s, ira %s', n4,n5,n6,n7)),
    ('professor B grava na propria escola', b1, 'insert em B permitido'),
    ('professor B nao acessa Storage legado (seed school)', b2 = false, 'legacy allowed='||b2);
  ...
  RESET ROLE;
  INSERT INTO ab_results VALUES
    ('papel resolvido por escola (dupla filiacao)', b1 AND b2, format('A=%s, B=%s', b1, b2)),
    ('direcao multi-escola le B quando e membro de B', n1 > 0, 'alunos B: '||n1);

  SELECT count(*) INTO n1 FROM public.school_memberships WHERE school_id = v_a AND user_id = v_tb;
  SELECT count(*) INTO n2 FROM public.school_registration_links WHERE token = 'ab-test-token-xyz' AND school_id = v_b;
  SELECT count(*) INTO n3 FROM public.ira_staleness WHERE school_id = v_a AND class_id = v_cls_b;
  INSERT INTO ab_results VALUES
    ('audiencia de A nao inclui professor de B', n1 = 0, 'memberships A p/ tb: '||n1),
    ('link de cadastro pertence so a B', n2 = 1, 'links: '||n2),
    ('staleness de IRA de B nao afeta A', n3 = 0, 'linhas cruzadas: '||n3);
