...
BEGIN
  ...
  -- Escola resolvida DENTRO da função (nunca vinda do cliente) e autorização por escola da linha.
  RETURN QUERY
    SELECT DISTINCT sg.grade_subject_id
      FROM public.student_grades sg
      JOIN public.grade_subjects gs ON gs.id = sg.grade_subject_id
     WHERE sg.grade_subject_id = ANY(_subject_ids)
       AND public.has_row_role(gs.school_id, ARRAY['admin','direction','teacher','staff']::app_role[]);
END;
$function$;
...
BEGIN
  ...
  -- Escolas diferentes: sempre rejeitado, inclusive para quem acessa as duas escolas.
  IF src.school_id IS DISTINCT FROM tgt.school_id THEN
    RAISE EXCEPTION 'Disciplinas de escolas diferentes nao podem ser consolidadas';
  END IF;

  IF NOT public.has_row_role(src.school_id, ARRAY['admin','direction']::app_role[]) THEN
    RAISE EXCEPTION 'Acesso negado';
  END IF;
