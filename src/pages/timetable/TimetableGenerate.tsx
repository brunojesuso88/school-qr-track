...
const GenerateContent = () => {
  ...
  const toggleShiftGroup = (shift: string) => {
    ...
    if (allSelected) {
      setSelectedClassIds(prev => prev.filter(id => !shiftClassIds.includes(id)));
    } else {
      setSelectedClassIds(prev => [...new Set([...prev, ...shiftClassIds])]);
    }
  };
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Escolha turmas individuais ou grupos por turno
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                    ...
                    <div key={shift} className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 ml-6">
                        {shiftClasses.map(cls => (
                          <div
                            key={cls.id}
                            className="flex items-center space-x-2 p-2 rounded-md border hover:bg-muted transition-colors"
                          >
                            <Checkbox
                              id={`class-${cls.id}`}
