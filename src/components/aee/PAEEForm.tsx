...
const appendText = (current: string, addition: string) =>
  !current.trim() ? addition : `${current.trimEnd()}\n• ${addition}`;
...
const AISuggestButton = ({ area, field, context, onPick }: AISuggestButtonProps) => {
  ...
  const fetchSuggestions = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('paee-suggest', {
        body: { area, field, context },
      });
  ...
  return (
    ...
    >
      ...
      <PopoverContent className="w-[min(460px,92vw)] p-2" align="end">
        <div className="flex items-center justify-between px-1 py-1.5">
          <span className="text-xs font-medium text-muted-foreground">
            Sugestões para {FIELD_LABELS[field]}
          </span>
          <Button
            type="button"
            variant="ghost"
            ...
            disabled={loading}
          >
            {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Gerar novas'}
          </Button>
        </div>
        <div className="max-h-72 overflow-y-auto space-y-1">
          ...
          {items.map((it, i) => (
            <button
              ...
              className="w-full text-left text-sm leading-snug rounded-md px-2 py-2 hover:bg-accent transition-colors"
            >
              {it}
            </button>
          ))}
