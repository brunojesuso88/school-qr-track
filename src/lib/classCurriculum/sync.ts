...
async function loadState(classId: string, series: HighSchoolSeries, schoolId: string): Promise<ClassCurriculumState> {
  ...
  const [matrix, classRow, gradeRes] = await Promise.all([
    ...
    supabase
      .from('grade_subjects')
      .select('id, name, weekly_classes, include_in_ira, slot_index, legacy_excluded, mapping_class_subject_id, sort_order, classification, ira_weight, curriculum_matrix_subject_id')
      .eq('school_id', schoolId)
      .eq('class_id', classId),
  ]);
